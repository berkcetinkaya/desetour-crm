...
-- ============================================================
-- File:    supabase_migration_civitatis_write_v7_passenger_completeness.sql
...
--          TO PRODUCTION, verified: security_definer=true, search_path=
--          public, owner=postgres, EXECUTE restricted to postgres/
--          service_role only — no anon/authenticated/PUBLIC)
--
-- Version: V7 — a NEW, separate migration file layered on top of the
--          already-applied V6, adding a passenger-completeness guard to
--          both the CREATE and MODIFICATION paths. STILL NOT APPROVED
--          FOR EXECUTION. This file does NOT rewrite or pretend V6 is
--          unapplied — supabase_migration_civitatis_write.sql (the V6
--          file) is left completely untouched and remains the accurate
--          historical record of what is live in production today; this
--          file's CREATE OR REPLACE FUNCTION is what advances it to V7
--          once reviewed and applied.
--
-- STATUS: V6 (the entire function body below, MINUS the two passenger-
-- completeness guards this file adds — see V7 CHANGELOG) is what is
-- currently live in production. This V7 file has NOT been executed
-- against any database. It exists so it can be read, reviewed, and
-- (once approved) applied manually — CREATE OR REPLACE FUNCTION is safe
-- to re-run directly over the already-applied V6. Nothing in this
-- repository currently calls the function it creates — see
-- api/civitatis/writeAdapter.js, which is fully written and tested but
-- not wired into any reachable HTTP endpoint or cron; writeAdapter.js
-- itself is UNCHANGED by V7 (or by V5/V6 before it).
--
...
-- confirming supabase_migration_civitatis_ingestion.sql AND
-- supabase_migration_civitatis_write.sql (V6) are already applied (they
-- are — V6 is live in production, verified per the STATUS note above).
-- Safe to re-run in full, and safe to run directly over the already-
-- applied V6: CREATE OR REPLACE FUNCTION and the REVOKE/GRANT statements
-- are all idempotent, and the function signature is unchanged from
-- V2/V4/V5/V6.
-- ============================================================
