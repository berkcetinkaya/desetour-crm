...
-- ============================================================
-- File:    supabase_migration_civitatis_write_v9_diagnostic_stage_instrumentation.sql
...
--          (V7 — ALREADY APPLIED TO PRODUCTION),
--          supabase_migration_civitatis_write_v8_passenger_array_type_guard.sql
--          (V8 — ALREADY APPLIED TO PRODUCTION, live-catalog-verified:
--          ingest_civitatis_booking's live body is EXACTLY the V8 body;
--          security_definer=true, search_path=public, owner=postgres,
--          EXECUTE restricted to postgres/service_role only)
--
-- Version: V9 — a NEW, separate migration file layered on top of the
--          already-applied V8, adding PURE DIAGNOSTIC INSTRUMENTATION
--          ONLY — a local stage-tracking variable and an expanded
--          GET STACKED DIAGNOSTICS capture inside the existing nested
--          EXCEPTION WHEN OTHERS handler. STILL NOT APPROVED FOR
--          EXECUTION. This file does NOT rewrite or pretend V8 is
--          unapplied — supabase_migration_civitatis_write_v8_passenger_
--          array_type_guard.sql (the V8 file) is left completely
--          untouched and remains the accurate historical record of what
--          is live in production today; this file's CREATE OR REPLACE
--          FUNCTION is what advances it to V9 once reviewed and applied.
--
...
-- V8's p_passengers guard and failed with "there is no unique or
-- exclusion constraint matching the ON CONFLICT specification". The
-- email_ingestions row (id fb7bd246-ab9d-492d-870c-40108d19e54f)
-- committed cleanly with this exact error_reason, proving the exception
-- was CAUGHT by this function's own nested EXCEPTION WHEN OTHERS
-- handler (Step 8) rather than escaping unhandled.
--
-- A full investigation exhausted every candidate identifiable from the
-- repository alone and was then DISPROVEN by live Supabase catalog
-- evidence at each step:
--   • public.ref_number_counters's ON CONFLICT (prefix, table_name,
--     year) target (inside public.next_ref_number, called from this
--     function's CREATE branch) — SUSPECTED, then DISPROVEN: live
--     pg_indexes confirms ref_number_counters_pkey is a genuine,
--     non-partial unique btree index over exactly those 3 columns.
--   • public.email_ingestions's ON CONFLICT (gmail_message_id) target
--     (this function's own Step 1) — ruled out: this exact call
--     returned a real ingestion_id, which is only possible if that
--     statement already succeeded.
--   • Every other write statement in the CREATE path (the customers
--     INSERT, the reservations INSERT, both reservation_guests INSERTs,
--     the activity_logs INSERT) — re-confirmed by direct inspection to
--     have NO ON CONFLICT clause at all, so none of them can be the
--     source of this specific error message.
--   • Hidden triggers on any of the six relevant tables — ruled out:
--     live pg_trigger inspection confirms the only non-internal
--     triggers anywhere on customers/reservations are BEFORE UPDATE
--     updated_at triggers; email_ingestions, ref_number_counters,
--     reservation_guests, and activity_logs have NONE.
--   • A stale/drifted live function body — ruled out: live
--     pg_get_functiondef confirms ingest_civitatis_booking's deployed
--     body is EXACTLY the V8 body, and next_ref_number's deployed body
--     is exactly as expected.
-- With every identifiable candidate exhausted and disproven, static
-- repository inspection alone cannot pinpoint the exact failing
-- statement any further. This migration does not guess at a fix — it
-- instruments the function so the NEXT retry attempt reports, in its
-- own audit trail, exactly which stage of the CREATE path Postgres was
-- executing when it raised this error, and every GET STACKED
-- DIAGNOSTICS field Postgres itself offers about that exception
-- (including PG_EXCEPTION_CONTEXT, which names the exact function and
-- line the error originated in) — turning the next real attempt into
-- conclusive evidence instead of another guess.
--
-- WHAT THIS MIGRATION DOES NOT DO
-- ─────────────────────────────────────────────────────────────
-- Zero business-logic change. Every branch condition, every write
-- statement's column list and values, every lock, every guard, every
-- RETURN shape's existing keys, and the overall event/reservation-state
-- matrix are byte-for-byte identical to V8. The EXCEPTION WHEN OTHERS
-- handler still does exactly what it always did (mark the SAME
-- email_ingestions row 'failed', roll back every business write
-- attempted since the Step 3 savepoint) — it now ALSO records more
-- about why, using information Postgres already computed for that
-- exception and would otherwise discard. No exception is caught that
-- wasn't already caught before; no exception is now swallowed that
-- would previously have propagated (Step 2b, still outside the nested
-- block exactly as in V6/V7/V8, is UNCHANGED and still not touched by
-- this instrumentation). No new table, column, index, or constraint is
-- created — v_stage and the GET STACKED DIAGNOSTICS variables are local
-- PL/pgSQL variables that exist only for the duration of one function
-- call; nothing about them is persisted anywhere except inside the
-- SAME error_reason TEXT column and the SAME returned JSONB shape this
-- function has always written to on failure.
--
-- STATUS: V8 (the entire function body below, MINUS the diagnostic
-- instrumentation this file adds — see V9 CHANGELOG) is what is
-- currently live in production, live-catalog-verified. This V9 file has
-- NOT been executed against any database. It exists so it can be read,
-- reviewed, and (once approved) applied manually — CREATE OR REPLACE
-- FUNCTION is safe to re-run directly over the already-applied V8.
-- Nothing in this repository currently calls the function through a
...
--
-- V9 CHANGELOG (what changed since the applied V8, and why — see "WHY
-- THIS MIGRATION EXISTS" above for the full incident writeup)
-- ─────────────────────────────────────────────────────────────
--   13. DIAGNOSTIC STAGE TRACKING + EXPANDED EXCEPTION DIAGNOSTICS. A new
--       local variable v_stage TEXT is set immediately before every
--       potentially-failing statement inside the nested BEGIN block
--       (Step 3 onward, all five Step 6 branches — not just CREATE), to
--       one of: nested_block_entry, booking_advisory_lock,
--       reservation_lookup, newer_event_check, booking_already_exists_
--       mark, modification_no_reservation_mark, passenger_completeness_
--       check, customer_resolution, customer_insert, tour_lookup,
--       reference_number_allocation, reservation_insert, passenger_
--       insert, ingestion_mark_processed, activity_log_insert, stale_
--       mark_processed, reservation_update, modification_passenger_
--       replace, unexpected_branch. Setting a local variable cannot
--       itself raise, so this adds no new failure mode. When EXCEPTION
--       WHEN OTHERS catches an error, it now also runs
--       GET STACKED DIAGNOSTICS to capture RETURNED_SQLSTATE,
--       CONSTRAINT_NAME, TABLE_NAME, COLUMN_NAME, PG_EXCEPTION_DETAIL,
--       PG_EXCEPTION_HINT, and PG_EXCEPTION_CONTEXT — all seven are
--       standard PL/pgSQL exception-block diagnostics items, valid only
--       inside an EXCEPTION handler (exactly where they are used here),
--       and none of them can ever contain application data (email
--       bodies, customer PII, passenger names) — they describe SQL
--       error metadata only (a SQLSTATE code, constraint/table/column
--       identifiers, and internal function/line call-stack text).
--       error_reason (the existing TEXT column, unchanged schema) is
--       now populated as "stage=<stage>; error=<original SQLERRM>;
--       sqlstate=<code>; constraint=<name or ->; table=<name or ->;
--       column=<name or ->; detail=<text or ->; hint=<text or ->" — the
--       original SQLERRM is ALWAYS preserved verbatim, never replaced
--       or summarized. The returned JSONB on failure gains two new
--       keys, 'stage' (the same stage string) and 'diagnostics' (a
--       nested object with sqlstate/constraint_name/table_name/
--       column_name/detail/hint/context — context is the one field
--       defensively bounded to its first 1000 characters, since it is
--       the only one of the seven that is a multi-line PL/pgSQL call
--       stack rather than a short identifier); every key this function
--       already returned on any result value is completely unchanged.
--
--       Explicitly UNCHANGED by V9: every V8/V7/V6 guarantee (the V8
--       p_passengers type guard, V7 passenger completeness, the V6
--       Gmail-message-scoped pg_try_advisory_xact_lock, the V5/V6
--       failed-retry state machine, the V4 event/reservation-state
--       matrix, the Step 3 booking-level advisory lock, customer
--       matching/creation behavior, financial field mapping, the
--       function's parameter signature, and the REVOKE/GRANT
--       service_role-only access control). The SAME email_ingestions
--       row (id fb7bd246-ab9d-492d-870c-40108d19e54f, gmail_message_id
--       1a0aedc69a2ef905) remains reclaimable exactly as V5/V6 always
--       allowed: its processing_status='failed' is retried by the SAME
--       Step 1 "ON CONFLICT (gmail_message_id) DO UPDATE ... WHERE
--       processing_status = 'failed'" logic, completely untouched by
--       this file — no manual deletion or reset of that row is
--       required or performed by this migration.
--
...
-- supabase_migration_civitatis_write.sql (V6),
-- supabase_migration_civitatis_write_v7_passenger_completeness.sql (V7),
-- AND supabase_migration_civitatis_write_v8_passenger_array_type_guard.sql
-- (V8) are already applied (they are, live-catalog-verified — see the
-- STATUS note above). Safe to re-run in full, and safe to run directly
-- over the already-applied V8: CREATE OR REPLACE FUNCTION and the
-- REVOKE/GRANT statements are all idempotent, and the function signature
-- is unchanged from V2/V4/V5/V6/V7/V8. This migration is pure
-- instrumentation — it does not require, and does not touch,
-- ref_number_counters or any other table.
-- ============================================================
...
DECLARE
  ...
  -- CHANGELOG above. None of these are ever persisted outside this one
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- V9: diagnostic-only capture. GET STACKED DIAGNOSTICS reads
    -- information Postgres already computed for the exception that was
    -- just caught — none of these seven items can ever contain
    -- application data (no email body, no customer/passenger PII): they
    -- describe SQL error metadata only — a SQLSTATE code, the
    -- constraint/table/column the engine associated with the error (if
    -- any — empty string when not applicable, never an error to ask
    -- for), and PL/pgSQL's own internal function/line call-stack text.
    -- This is the ONLY new statement this migration adds to the
    -- EXCEPTION handler; it cannot itself raise.
    ...
    -- error_reason keeps the ORIGINAL SQLERRM verbatim (never replaced
    -- or summarized) and gains the diagnostic stage plus the compact
    -- identifier-shaped diagnostics fields, in the exact
    -- "stage=<stage>; error=<original SQLERRM>" format requested, plus
    -- sqlstate/constraint/table/column/detail/hint. PG_EXCEPTION_CONTEXT
    -- (the one multi-line, potentially long field of the seven) is
    -- deliberately NOT appended here — it goes into the returned JSONB
    -- only (bounded to 1000 characters), to keep this audit column
    ...
    RETURN jsonb_build_object(
      ...
      'ingestion_id', v_ingestion_id,
      'error', v_error_text,
      'stage', COALESCE(v_stage, 'unknown'),  -- V9: diagnostic only
      'diagnostics', jsonb_build_object(      -- V9: diagnostic only
...
COMMENT ON FUNCTION public.ingest_civitatis_booking IS
  ...
  'On an unexpected error, the failed email_ingestions row''s '
  'error_reason and the returned JSON both now also record which '
  'internal processing stage was last entered and Postgres''s own '
  'GET STACKED DIAGNOSTICS fields (sqlstate/constraint/table/column/ '
  'detail/hint/a bounded context) for that exact exception, purely for '
  'diagnosis — no business behavior changes (V9). '
